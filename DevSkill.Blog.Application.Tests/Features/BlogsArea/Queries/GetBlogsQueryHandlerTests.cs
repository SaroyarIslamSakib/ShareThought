...
public class GetBlogsQueryHandlerTests
{
    ...
    private GetBlogsQueryHandler _handler;
    ...
    public void Setup()
    {
        ...
        _handler = _moq.Create<GetBlogsQueryHandler>();
    }
    ...
    [Test]
    public async Task Handle_ValidQuery_ReturnsMappedBlogs()
    {
        // Arrange
        ...
        first.Title.ShouldBe("Public Blog 1");
        first.OwnerName.ShouldBe("User One");
        first.OwnerEmail.ShouldBe("user1@test.com");
        first.TotalPosts.ShouldBe(3);
        first.BlogSlug.ShouldBe("public-blog-1");

        _blogAreaRepositoryMock.Verify(x =>
            x.GetPagedBlogsAsync(
                query.PageIndex,
                query.PageSize,
                query.SearchText,
